...
class WorkflowEngine:
    ...
    def _extract_audio_step(self, settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        output_filename = f"{self.config.input_file.stem}_extracted.{settings['format']}"
    ...
    def _process_audio_step(self, settings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if settings.get("increase_volume", False):
            volume_output = self.config.output_dir / f"{current_file.stem}_volume.{current_file.suffix[1:]}"
        ...
        if settings.get("normalize_audio", True):
            norm_output = self.config.output_dir / f"{current_file.stem}_normalized.{current_file.suffix[1:]}"
