...
class TestWorkflowEngine:
    ...
    def test_execute_workflow_audio_file(self, mock_summarize, mock_transcribe, tmp_path):
    ...
    @patch('core.workflow.transcribe_run')
    @patch('core.workflow.summarize_run')
    def test_execute_workflow_video_file(self, mock_summarize, mock_transcribe, 
    ...
    def test_execute_workflow_transcript_file(self, mock_summarize, tmp_path):
        ...
        with patch('core.utils.validation.validate_workflow_input') as mock_validate:
            ...
            # Only summarization should run
            assert "summarize" in results
            assert "extract_audio" not in results
            assert "transcribe" not in results
            mock_summarize.assert_called_once()
...
class TestWorkflowSteps:
    ...
    @patch('core.workflow.extract_audio_from_video')
    def test_extract_audio_step(self, mock_extract, tmp_path):
    ...
    @patch('core.workflow.increase_audio_volume')
    @patch('core.workflow.normalize_loudness')
    @patch('core.workflow.convert_audio_format')
    def test_process_audio_step(self, mock_convert, mock_normalize, mock_volume, tmp_path):
        ...
        volume_file = tmp_path / "output" / "input_volume.mp3"
        norm_file = tmp_path / "output" / "input_volume_normalized.mp3"
        converted_file = tmp_path / "output" / "input_volume_normalized.wav"
        
        mock_volume.return_value = volume_file
        mock_normalize.return_value = norm_file  
    ...
    @patch('core.workflow.transcribe_run')
    @patch('core.workflow.ensure_wav16k_mono')
    def test_transcribe_step(self, mock_ensure_wav, mock_transcribe, tmp_path):
        ...
        transcript_file = tmp_path / "output" / "transcript.json"
    ...
    def test_summarize_step(self, mock_summarize, tmp_path):
        ...
        # Mock summarization
        summary_file = tmp_path / "output" / "summary.json"
        mock_summarize.return_value = summary_file
...
class TestWorkflowErrorHandling:
    ...
    def test_workflow_step_error(self, tmp_path):
